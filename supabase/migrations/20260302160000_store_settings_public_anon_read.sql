-- Permite que usuários anônimos vejam as configurações públicas da loja (logo, header, etc.)
-- A tabela store_settings continua restrita a admins (RLS: Only admins can read).
-- A view passa a rodar como SECURITY DEFINER: executa com privilégios do dono da view,
-- assim anon pode SELECT na view (e recebe os dados) sem precisar de permissão na tabela.
