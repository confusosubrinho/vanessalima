...
function getMessageForRoute(pathname: string): string {
  ...
  if (pathname === '/carrinho') return 'Olá! Tenho itens no carrinho e gostaria de ajuda para finalizar minha compra.';
  if (pathname === '/checkout') return 'Olá! Estou finalizando uma compra e preciso de ajuda.';
  ...
  if (pathname === '/trocas') return 'Olá! Gostaria de fazer uma troca ou devolução.';
  if (pathname === '/pedido-confirmado') return 'Olá! Acabei de fazer um pedido e gostaria de confirmar os detalhes.';
...
export function WhatsAppFloat() {
  ...
  return (
    <a
      ...
      className="fixed bottom-24 right-4 sm:bottom-6 sm:right-6 z-50 bg-[#25D366] hover:bg-[#20bd5a] text-white p-4 sm:p-4 rounded-full shadow-2xl transition-all hover:scale-110 group md:bottom-6"
