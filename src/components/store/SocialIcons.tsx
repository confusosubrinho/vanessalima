...
// SVG icons for default platforms
const PLATFORM_ICONS: Record<string, React.FC<{ className?: string }>> = {
  ...
  kwai: ({ className }) => (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm3.5 14.5l-3.5-2.5-3.5 2.5 1-4L6 9.5l4-.5L12 5l2 4 4 .5-3.5 3 1 4z"/></svg>
  ),
};

function getPlatformKey(name: string): string | null {
  ...
  if (lower.includes('kwai') || lower.includes('kawai')) return 'kwai';
  return null;
}

function SocialIcon({ link, className }: { link: SocialLink; className?: string }) {
  if (link.icon_type === 'custom' && link.icon_image_url) {
    return (
      <img 
  ...
  const platformKey = getPlatformKey(link.name);
  const IconComponent = platformKey ? PLATFORM_ICONS[platformKey] : null;
...
export function SocialIcons({ className, iconClassName, linkClassName }: SocialIconsProps) {
    ...
    <div className={cn("flex gap-4", className)}>
      {links.map((link) => (
        <a
          ...
          className={cn("hover:text-primary transition-colors", linkClassName)}
          title={link.name}
        >
...
export { SocialIcon, getPlatformKey, PLATFORM_ICONS };
